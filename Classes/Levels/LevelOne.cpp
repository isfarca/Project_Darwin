...
void LevelOne::InitBoxesWithCollider()
{
	...
	greyBoxCollider[0]->setPosition(greyBoxSprite[0]->getPositionX() - 90, greyBoxSprite[0]->getPositionY());
	greyBoxCollider[0]->setScaleY(3);
	greyBoxCollider[0]->setVisible(false);
	this->addChild(greyBoxCollider[0], 0);
	// Top collider.
	greyBoxCollider[1] = Sprite::create("Collider.png");
	greyBoxCollider[1]->setPosition(greyBoxSprite[0]->getPositionX(), greyBoxSprite[0]->getPositionY() + 90);
	greyBoxCollider[1]->setScaleX(3);
	greyBoxCollider[1]->setVisible(false);
	this->addChild(greyBoxCollider[1], 0);
	// Right collider.
	greyBoxCollider[2] = Sprite::create("Collider.png");
	greyBoxCollider[2]->setPosition(greyBoxSprite[0]->getPositionX() + 90, greyBoxSprite[0]->getPositionY());
	greyBoxCollider[2]->setScaleY(3);
	greyBoxCollider[2]->setVisible(false);
	this->addChild(greyBoxCollider[2], 0);
	// Bottom collider.
	greyBoxCollider[3] = Sprite::create("Collider.png");
	greyBoxCollider[3]->setPosition(greyBoxSprite[0]->getPositionX(), greyBoxSprite[0]->getPositionY() - 90);
	greyBoxCollider[3]->setScaleX(3);
	greyBoxCollider[3]->setVisible(false);
	this->addChild(greyBoxCollider[3], 0);
}
...
void LevelOne::PlayerBoxesCollisionDetection()
{
	...
	{
		// Collider for 'greyBoxSprite[0]'.
		greyBoxCollider[0]->getBoundingBox(), // Left.
		greyBoxCollider[1]->getBoundingBox(), // Top.
		greyBoxCollider[2]->getBoundingBox(), // Right.
		greyBoxCollider[3]->getBoundingBox() // Bottom.
	};

	// Collision detection with 'greyBoxSprite[0]'.
	if (playerLeftColliderRect.intersectsRect(greyBoxColliderRect[2])) // Left <-> Right.
		moveLeft = false;
	else if (playerTopColliderRect.intersectsRect(greyBoxColliderRect[3])) // Top <-> Bottom.
		this->stopAction(jumpTo);
	else if (playerRightColliderRect.intersectsRect(greyBoxColliderRect[0])) // Right <-> Left.
		moveRight = false;
	else if (playerBottomColliderRect.intersectsRect(greyBoxColliderRect[1])) // Bottom <-> Top.
		falling = false;
	else
		falling = true;
}
